// admin-cli.js - VERSÃO AJUSTADA PARA RENDER (Prepared Statements)

require('dotenv').config();
const { query } = require('./db');
const bcrypt = require('bcryptjs'); // ✅ Corrigido: bcryptjs ao invés de bcrypt
  ...
  try {
    const sqlText = `
      SELECT 
        ...
        perfil, 
        ativo, 
        CONVERT(varchar, data_criacao, 103) as criado_em 
      FROM usuarios_api
        ...
        rl.question('Senha: ', pass => {
          rl.question('Perfil (admin/consultor): ', async perfil => {
            try {
              ...
              // ✅ CORRIGIDO: Prepared Statements ao invés de concatenação
              const sqlText = `
                INSERT INTO usuarios_api (usuario, senha_hash, email, perfil, ativo, data_criacao, dashboards)
                VALUES (@usuario, @senhaHash, @email, @perfil, 1, GETDATE(), 'todos')
              `;
              
              await query(sqlText, [
                { name: 'usuario', type: sql.NVarChar, value: user },
                { name: 'senhaHash', type: sql.NVarChar, value: hash },
    ...
    rl.question('Qual usuário? ', user => {
      rl.question('Nova senha: ', async pass => {
        try {
          ...
          const sqlText = `
            UPDATE usuarios_api 
            SET senha_hash = @senhaHash, data_atualizacao = GETDATE() 
    ...
    rl.question('Qual usuário? ', user => {
      rl.question('Novo perfil (admin/consultor): ', async perfil => {
        try {
          const sqlText = `
            UPDATE usuarios_api 
            SET perfil = @perfil, data_atualizacao = GETDATE() 
            WHERE usuario = @usuario
          `;
    ...
    rl.question('Qual usuário? ', user => {
      rl.question('Ativar (1) ou Desativar (0)? ', async st => {
        try {
          ...
          const sqlText = `
            UPDATE usuarios_api 
            SET ativo = @ativo, data_atualizacao = GETDATE() 
            WHERE usuario = @usuario
          `;
          
          await query(sqlText, [
            { name: 'usuario', type: sql.NVarChar, value: user },
            { name: 'ativo', type: sql.Int, value: bit }
          ]);
